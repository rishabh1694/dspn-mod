...
def main():
    ...
    def run(net, loader, optimizer, train=False, epoch=0, pool=None):
        ...
        for i, sample in enumerate(loader, start=epoch * iters_per_epoch):
            ...
            tracked_last = tracker.update(
                "{}_last".format(prefix), set_loss[-1].item())
            tracked_loss = tracker.update(
            ...
            if args.show and not train:
                ...
                for j, (s, ms) in enumerate(zip(progress, masks)):
                    ...
                    s, ms = utils.scatter_masked(
                        ...
                        threshold=0.5 if args.dataset.startswith(
                            "clevr") else None,
                    )
    ...
    for epoch in range(args.epochs):
        ...
        with mp.Pool(10) as pool:
            if not args.eval_only:
                run(net, train_loader, optimizer,
                    train=True, epoch=epoch, pool=pool)
            if not args.train_only:
                run(net, test_loader, optimizer,
                    train=False, epoch=epoch, pool=pool)
