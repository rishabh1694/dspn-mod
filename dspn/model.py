...
def build_net(args):
    ...
    if use_convolution:
        ...
        # input_encoder = None
        input_encoder = FSEncoder(set_channels, latent_dim, hidden_dim)
        input_encoder.load_state_dict(torch.load(
            "/home/rishabhgupta/pytorch-deep-sets/saved_model"))
        input_encoder.eval()
...
class Net(nn.Module):
    ...
    def forward(self, input, target_set, target_mask):
        if self.input_encoder is None:
            ...
            latent_repr = self.input_encoder(target_set, target_mask)
